...
public class ParticleEngine_v1 extends JPanel implements KeyListener {
	...
	*/
	...
	public static abstract class AnyBorderLine extends ParticleEngineGUI {
		...
		public static class HorrizontalRepellingBorderLine extends AnyBorderLine implements AnyVariableListener {
			...
			public void repel(AnyParticleInstance particle) {
				if(!particle.type.isYmoving() || !particle.type.isMoving()) { return; }
	...
	public void handleParticles() {
		...
		particleMutex.dlock("ParticleEngine Particles Handler");
		GUImutex.dlock("ParticleEngine Particles Handler");
		for(AnyParticleType cpt : allTypes) { cpt.handlePendings(); }
		GUImutex.dunlock("ParticleEngine Particles Handler");
		for(AnyParticleInstance cpi : allParticles) { cpi.handleFinity(); }
		for(AnyParticleInstance cpi : allParticles) { cpi.lbUpdate(this); }
		particleMutex.dunlock("ParticleEngine Particles Handler");
	}
	public void lbUpdate() {
		if(!isrunning) { return; }
		...
		particleMutex.dlock("ParticleEngine lbUpdate");
		GUImutex.dlock("ParticleEngine lbUpdate");
		handlePendings();
		particleMutex.dunlock("ParticleEngine lbUpdate");
		for(ParticleEngineGUI gui : allGUI) { gui.lbUpdate(); }
		GUImutex.dunlock("ParticleEngine lbUpdate");
	}
	...
	public void handlePendings() {
		...
		for (int i=0;i!=removedGUI.size();i++) {
			...
			if(AnyParticleType.class.isInstance(element)) { allTypes.remove(element); }
			if(AnyAudience.class.isInstance(element)) { allAudienceTargets.remove(element); }
			if(AnyBorderLine.class == element.getClass()) { allBorderLines.remove(element); }
			//if(AnySensor.class.isInstance(element)) { allSensors.remove(element); }
			if(AnyVariable.class.isInstance(element)) { allVariables.remove(element); }
	...
	public void lbPaint(Graphics2D target) {
		...
		particleMutex.dlock("ParticleEngine lbPaint");
		...
		particleMutex.dunlock("ParticleEngine lbPaint");
		GUImutex.dlock("ParticleEngine lbPaint");
		for(int i=allGUI.size();i!=0;i--) { allGUI.get(i-1).lbPaint(target); }
		GUImutex.dunlock("ParticleEngine lbPaint");
